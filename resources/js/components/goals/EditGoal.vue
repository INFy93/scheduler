<template>
  <header class="bg-white dark:bg-gray-800 shadow">
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      ...
      >
        Редактировать задачу: {{ goal.name }}
      </h2>
    </div>
  </header>
  <div class="py-12">
                ...
                <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                  ...
                  <div class="mt-1">
                    <select
                      ...
                      id="user"
                      class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                      v-model="goal.performer_id"
                    >
                      <option
                        v-for="user in users"
                        :key="user.id"
                        :value="user.id"
                        :selected="user.id == goal.performer_id"
                      >
                        {{ user.name }}
                      </option>
                    </select>
                ...
                <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                  ...
                  <div class="mt-1">
                    <select
                      ...
                      class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                    >
                      <option value="telega" selected="selected">
                        Telegram
                      </option>
...
export default {
  ...
  setup(props) {
    const { goal, errors, getGoal, updateGoal } = useGoals();
    const { users, current_user, getAllUsers } = useUsers();
    const today = new Date().toISOString().split("T")[0];

    onMounted(async() => {
      await getGoal(props.id);
      await getAllUsers();
    });
